...
import { clearWorkbook, loadWorkbook, saveWorkbook as persistWorkbook, type StorageMode } from './db';
...
import { createSampleWorkbook } from './sample';
...
function isDemoLocation(): boolean {
  ...
  return window.location.pathname.replace(/\/+$/, '') === '/demo' || query.get('demo') === '1';
}

function setRouteMetadata(mode: StorageMode): void {
  const demo = mode === 'demo';
  const title = demo ? 'Demo — Source Trail Workbook' : 'Source Trail Workbook — Trace claims to sources';
  const description = demo
    ? 'Explore three completed humanities research trails without changing your real workbook.'
    : 'Help students record how a search, checked source, and quotation support a research claim.';
  const canonical = demo ? 'https://source-trail-workbook.sociobot.in/demo/' : 'https://source-trail-workbook.sociobot.in/';
...
function demoBanner(): string {
  if (storageMode !== 'demo') return '';
  return `<aside class="demo-banner" aria-label="Demo mode">
    <p><strong>Demo — sample data, nothing is saved to your real workbook.</strong></p>
    <div><button class="text-button" data-action="reset-demo">Reset demo</button><button class="button button-small" data-action="start-real">Start for real</button></div>
  </aside>`;
}
...
function header(): string {
  return `
    ...
    <header class="site-header">
      <a class="identity" href="/" data-route="home" aria-label="Source Trail Workbook home">${mark}<span>Source Trail Workbook</span></a>
      <nav class="site-nav" aria-label="Main navigation"><a href="/" data-route="home">Home</a><a href="/?demo=1" data-route="demo" ${storageMode === 'demo' ? 'aria-current="page"' : ''}>Demo</a><a href="/privacy/">Privacy</a></nav>
      ${screen === 'workbook' ? `
  ...
  return `${header()}
    <main id="main-content" class="landing" tabindex="-1">
      <section class="hero-copy" aria-labelledby="hero-heading">
        <p class="eyebrow">A browser worksheet for research evidence</p>
        ...
        <ul class="plain-facts" aria-label="Workbook facts"><li>Free to use.</li><li>Works offline after the first visit.</li><li>Workbook content stays in this browser.</li></ul>
      </section>
      <figure class="hero-figure">
        ...
        <figcaption>See how a question becomes a supported claim.</figcaption>
      </figure>
      <section class="method" aria-labelledby="method-heading">
        <p class="eyebrow">Three steps</p>
        <h2 id="method-heading">How to record a research trail</h2>
        <ol class="method-list">
          <li><span>01</span><div><h3>Record the search</h3><p>Keep the exact query and one result you passed over.</p></div></li>
          <li><span>02</span><div><h3>Check the source</h3><p>Name who made it, what supports it, and what limits it.</p></div></li>
      ...
      <aside class="teacher-note">
        ...
        <h2>Give students a blank research-trail template</h2>
        <p>Add the course prompt and instructions, then export a blank template. Students import it and return their completed workbook.</p>
      </aside>
    </main>
    ${footer()}
    ${globalOverlays()}`;
...
function trailEditor(trail: Trail, index: number): string {
  ...
  return `<article class="trail-sheet" aria-labelledby="trail-heading">
    <div class="sheet-heading">
      <div><p class="eyebrow">Trail ${String(index + 1).padStart(2, '0')}</p><h2 id="trail-heading" tabindex="-1">${escapeHtml(trailName(trail, index))}</h2></div>
    ...
    <form id="trail-form" novalidate>
      ...
      <section class="form-section" aria-labelledby="section-source">
        <div class="section-marker">2</div><div class="section-body">
          <div class="section-heading"><div><p class="eyebrow">Source record</p><h3 id="section-source">What did you inspect?</h3></div><p>Source details help locate a source. They do not prove quality.</p></div>
...
function globalOverlays(): string {
  return `<input class="visually-hidden" type="file" id="import-file" aria-label="Choose a workbook JSON file" accept="application/json,.json" />
    ...
    ${screen === 'workbook' ? `<div class="save-state" id="save-state" role="status">${storageAvailable ? 'Saved locally' : 'Local save unavailable — export to keep your work'}</div>` : ''}
...
async function saveNow(action?: string): Promise<void> {
  ...
  try {
    await persistWorkbook(workbook, storageMode);
...
async function exportFile(kind: 'json' | 'template' | 'csv' | 'citations'): Promise<void> {
  if (!workbook) return;
  // Keep the download inside the click's user activation. Saving runs in
  // parallel from the same in-memory workbook, so no export data is lost.
  void saveNow();
  if (kind === 'json') download(JSON.stringify({ exportedAt: now(), workbook }, null, 2), filename('json'), 'application/octet-stream');
  if (kind === 'template') download(JSON.stringify({ exportedAt: now(), workbook: makeTemplate(workbook) }, null, 2), filename('template.json'), 'application/octet-stream');
...
async function enterDemo(pushHistory = true): Promise<void> {
  storageMode = 'demo';
  if (pushHistory) window.history.pushState({ route: 'demo' }, '', '/?demo=1');
  setRouteMetadata('demo');
  const savedDemo = await loadWorkbook('demo');
  workbook = savedDemo ? parseWorkbook(savedDemo) : createSampleWorkbook();
  currentTrailId = workbook.trails[0]?.id ?? '';
  screen = 'workbook';
  await persistWorkbook(workbook, 'demo');
  render();
  focusRouteHeading('Demo — completed sample workbook');
}
...
async function leaveDemo(): Promise<void> {
  ...
  setRouteMetadata('real');
  const saved = await loadWorkbook('real');
  workbook = saved ? parseWorkbook(saved) : null;
  currentTrailId = workbook?.trails[0]?.id ?? '';
  screen = 'start';
  render();
  focusRouteHeading('Home — Source Trail Workbook');
}
...
async function handleAction(action: string, element: HTMLElement): Promise<void> {
  ...
  if (action === 'close-workbook') {
    if (storageMode === 'demo') await leaveDemo();
    else { await saveNow(); screen = 'start'; render(); }
  }
  ...
  if (action === 'start-real') await leaveDemo();
}

document.addEventListener('click', (event) => {
  const routeLink = (event.target as HTMLElement).closest<HTMLAnchorElement>('[data-route="demo"]');
  if (routeLink) {
    event.preventDefault();
    void enterDemo();
    return;
  }
  const homeLink = (event.target as HTMLElement).closest<HTMLAnchorElement>('[data-route="home"]');
  if (homeLink && storageMode === 'demo') {
    event.preventDefault();
    void leaveDemo();
    return;
  }
  const internalLink = (event.target as HTMLElement).closest<HTMLAnchorElement>('a[href]');
  if (internalLink && storageMode === 'demo') {
    const destination = new URL(internalLink.href, window.location.href);
    if (destination.origin === window.location.origin && !destination.searchParams.has('demo') && destination.pathname.replace(/\/+$/, '') !== '/demo') {
      event.preventDefault();
      void clearWorkbook('demo').then(() => { window.location.href = destination.href; });
      return;
    }
  }
...
window.addEventListener('popstate', () => {
  const nextMode: StorageMode = isDemoLocation() ? 'demo' : 'real';
  if (nextMode === 'demo') void enterDemo(false);
  else {
    storageMode = 'real';
    setRouteMetadata('real');
    void loadWorkbook('real').then((saved) => {
      workbook = saved ? parseWorkbook(saved) : null;
      screen = 'start';
      currentTrailId = workbook?.trails[0]?.id ?? '';
      render();
...
async function init(): Promise<void> {
  try {
    const saved = await loadWorkbook(storageMode);
    workbook = saved ? parseWorkbook(saved) : storageMode === 'demo' ? createSampleWorkbook() : null;
    if (workbook && storageMode === 'demo' && !saved) await persistWorkbook(workbook, 'demo');
    currentTrailId = workbook?.trails[0]?.id ?? '';
    if (storageMode === 'demo') {
      screen = 'workbook';
    }
    setRouteMetadata(storageMode);
  } catch {
    storageAvailable = false;
  }
  render();
